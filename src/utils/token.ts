...
// Test function to validate token creation and verification
export async function testTokenCreation() {
  const testPayload = { id: 'test-user' };
  try {
    ...
    console.log('Test token creation results', {
      accessValid: accessResult.valid,
      refreshValid: refreshResult.valid,
    });
  } catch (error) {
    console.error('Token creation/verification test failed', { error: String(error) });
  }
}
